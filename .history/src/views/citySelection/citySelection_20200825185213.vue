...
<div>
  <div class="w960">
    <div class="mg-lr10">
      ...
      <div class="fl mg-t20 fl-wrap t-center colg fz14 j-between" v-for="(item,index) in city1" :key="index">
        <!-- 左侧得字母 跟右侧城市对应得城市名-->
        <div class="index-box mg-t10">{{index}}</div>
        <div class="fl citys-box fl-wrap">
          <div v-for="(item1) in item" :key="item1.id" @click="citys1(item)" class="c-pointer fl ">
            <!-- 右侧得城市名 跟左侧字母对应的城市-->
            <div class="item1-box  mg-t10 mg-l15">{{item1.name}}</div>
          </div>
        </div>
      </div>
