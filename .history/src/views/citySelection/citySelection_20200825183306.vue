  ...
  <div class="w960">
    <div class="mg-lr10">
      ...
      <div class="fl fz16 mg-b20">
        <div>按拼音首字母选择：</div>
        <div v-for="item in tetter" :key="item.id">
          <div @click="citys1(item)" class=" c-pointer mg-l15 ">{{item.id}}</div>
        </div>
      </div>
      <div v-for="(item,index) in city1" :key="index">
        <div @click="citys1(item)" class=" c-pointer mg-l15 ">{{item}}</div>
      </div>
    </div>
  </div>
</div>
...
import tetter from '../../可能要用的js文件/tetter'
export default {
  name: "",
  props: {},
  components: {},
