  ...
  <div class="w960">
    <div class="mg-lr10">
      ...
      <div class="fl fz16 mg-b20">
        <div>按拼音首字母选择：</div>
        <div>
          <a href="javascript:void(0)" rel="external nofollow" @click="goAnchor('#anchor-'+index)" v-for="index in 20">{{index}}</a>
        </div>
        <div v-for="(item,index) in letter" :key="index">
          <div :id="'anchor-'+index" class="item c-pointer mg-l15">{{item.id}}</div>
        </div>
      </div>

      <div class="fl mg-t20 fl-wrap fz14 j-between" v-for="(item,index) in city1" :key="index">
        <!-- 左侧得字母 跟右侧城市对应得城市名-->
        <div class="index-box t-center mg-t10">{{index}}</div>
