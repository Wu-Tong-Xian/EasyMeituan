    ...
    <div class="mg-lr10">
                ...
                <el-dropdown-menu slot="dropdown">
                  <div class="name-box fl-wrap fl">
                    <div v-for="item in province" :key="item.id " class>
                      <div @click="citys" class="fz12 c-pointer colg mg10">{{item.name}}</div>
                    </div>
                  </div>
                </el-dropdown-menu>
              </el-dropdown>
      ...
      <div class="fl mg-b20">
        ...
        <div v-for="item in hots" :key="item.id">
          <div v-if="index <=3" @click="citys" class="mg-l10 c-pointer fz14 colg">{{item.province}}</div>
          <div v-if="index > 3 && index < 11" @click="citys" class="mg-l10 c-pointer fz14 colg">{{item.name}}</div>
        </div>
      </div>
      <el-divider></el-divider>
      <div class="fl mg-b20">
        <div>最近访问：</div>
        <div v-for="item in hots" :key="item.id">
          <div @click="citys" class="mg-l10 c-pointer fz14 colg">{{item.province}}</div>
        </div>
      </div>
    </div>
  </div>
